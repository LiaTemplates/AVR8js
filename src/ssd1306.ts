 ...
 *
 * For a full-blown Arduino simulator with SSD1306 and many other parts, check out Wokwi
 * at https://wokwi.com/
 */
  ...
  toImageData(target: ImageData) {
    ...
    for (let y = 0; y < height; y++) {
      for (let x = 0; x < width; x++) {
        ...
        const pixValue =
          pixels[pixIndex] & (1 << virtualY % 8) ? !invert : invert
  ...
  processCommand(value: number) {
    ...
    switch (commandCode) {
      ...
      case CMD_VERTICAL_AND_LEFT_HORIZONTAL_SCROLL:
        this.scrollRight =
          commandCode === CMD_VERTICAL_AND_RIGHT_HORIZONTAL_SCROLL
        this.scrollStartPage = this.currentCommand[2] & 0x7
        this.scrollSpeed = (this.currentCommand[3] & 0x7) as number3bits
        this.scrollEndPage = this.currentCommand[4] & 0x7
      ...
      default:
        ...
        if (commandCode >= 0x10 && commandCode <= 0x1f) {
          this.activeColumn =
            (this.activeColumn & 0x0f) | ((commandCode & 0x0f) << 4)
          break
        }
  ...
  private scroll() {
    ...
    for (let page = this.scrollStartPage; page <= this.scrollEndPage; page++) {
      ...
      if (this.scrollRight) {
        ...
        this.pixels.set(
          this.pixels.slice(pageStart, pageStart + pageSize - 1),
          pageStart + 1
        )
        ...
        this.pixels.set(
          this.pixels.slice(pageStart + 1, pageStart + pageSize),
          pageStart
        )
    ...
    if (this.scrollVertical) {
      this.scrollOffset =
        (this.scrollOffset + this.verticalOffset) % this.height
    }
  }
